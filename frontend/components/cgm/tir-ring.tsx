 ...
 */
export const TIRRing = ({tir, tbr, tar, label = "Time in range · 14 d", hint, className}: TIRRingProps) => {
    const segments = [
    ...
    const legend = [
        {color: GLUCOSE_ZONE_COLOR_VAR.high, label: "Above 180 (high)", value: tar},
        {color: GLUCOSE_ZONE_COLOR_VAR.inRange, label: "70–180 (healthy)", value: tir},
        {color: GLUCOSE_ZONE_COLOR_VAR.low, label: "Below 70 (low)", value: tbr},
    ];
            ...
            <CardContent className="flex items-center gap-5">
                ...
                <div className="flex-1">
                    <div className="mb-2.5 flex items-center gap-1">
                        <p className="text-[11px] font-bold tracking-[0.08em] text-muted-foreground uppercase">{label}</p>
                        {hint && <InfoHint label="Time in range">{hint}</InfoHint>}
                    </div>
                    {legend.map((row) => (
                        <div
                            key={row.label}
